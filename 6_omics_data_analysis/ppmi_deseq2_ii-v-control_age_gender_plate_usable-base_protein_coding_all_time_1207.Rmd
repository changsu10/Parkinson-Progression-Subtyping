...
# Subset metadata to exclude subtypes I/III.
df_meta <- df_meta[df_meta$Subtype != "I",]
df_meta <- df_meta[df_meta$Subtype != "III",]
...
dds$Subtype <- factor(dds$Subtype, levels=c('Control', 'II'))

# Differential expression analysis.
dds <- DESeq(dds)
de_results <- results(dds, 
                      #alpha = 0.05, # Adjusted p-value threshold for significance.
                      contrast = c("Subtype","II","Control")) # Which level as denominator? Case then control.
de_results
...
write.csv(de_results, 'dge_ii_vs_hc.csv')
```
