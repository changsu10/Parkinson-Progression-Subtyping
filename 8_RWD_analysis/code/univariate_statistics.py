...
def statistics_from_df(df, out_file):
    ...
    print(df.columns)
    df1 = df.loc[df['ND'] >= 1, :]
    df0 = df.loc[df['ND'] <= 0, :]  # may have -1 as censoring

    # Caution: not comparing race?
    index_cnt = ['No.', 'age_of_PD']
    ...
    columns = ['Overall_PD', 'ND', 'Others', 'p-value']
    rdf = pd.DataFrame(columns=columns, index=index_cnt + index_sex + index_race + index_risk)

    rdf.at['No.', 'Overall_PD'] = str(len(df))
    rdf.at['No.', 'ND'] = '{} ({:.2f}%)'.format(len(df1), len(df1) * 100.0 / len(df))
    rdf.at['No.', 'Others'] = '{} ({:.2f}%)'.format(len(df0), len(df0) * 100.0 / len(df))

    rdf.at['age_of_PD', 'Overall_PD'] = ASTD_IQR(df['age_of_PD'])
    rdf.at['age_of_PD', 'ND'] = ASTD_IQR(df1['age_of_PD'])
    rdf.at['age_of_PD', 'Others'] = ASTD_IQR(df0['age_of_PD'])
    rdf.at['age_of_PD', 'p-value'] = t_test(df1['age_of_PD'], df0['age_of_PD'])

    # sex, Chi-square test of independence
    rdf.at['sex', 'p-value'] = chi2(df1['sex'], df0['sex'])
    for c, name in {0: 'female', 1: 'male'}.items():
        rdf.at['sex-{}-{}'.format(c, name), 'Overall_PD'] = '{} ({:.2f}%)'.format((df['sex'] == c).sum(),
                                                                                   (df['sex'] == c).sum() * 100.0 / len(
                                                                                       df))
        rdf.at['sex-{}-{}'.format(c, name), 'ND'] = '{} ({:.2f}%)'.format((df1['sex'] == c).sum(),
    ...
    for c, name in {5: 'white', 0: 'other', 3: 'black', 2: 'asian'}.items():
        rdf.at['race-{}-{}'.format(c, name), 'Overall_PD'] = '{} ({:.2f}%)'.format((df['race'] == c).sum(),
                                                                                    (df[
                                                                                         'race'] == c).sum() * 100.0 / len(
                                                                                        df))
        rdf.at['race-{}-{}'.format(c, name), 'ND'] = '{} ({:.2f}%)'.format((df1['race'] == c).sum(),
    ...
    for risk in index_risk:
        ...
        rdf.at[risk, 'Overall_PD'] = '{} ({:.2f}%)'.format((df[risk] >= 1).sum(),
                                                            (df[risk] >= 1).sum() * 100.0 / len(df))
        rdf.at[risk, 'ND'] = '{} ({:.2f}%)'.format((df1[risk] >= 1).sum(),
