...
def final_eval_ml_CV_revise_traintest(model, args,
    ...
    # # tem print zhx
    # Unbalanced_features_SMD_value = SMD_ALL[1][SMD_ALL[1] > SMD_THRESHOLD]
    # Unbalanced_features_SMD_names = feature_name[np.where(SMD_ALL[1] > SMD_THRESHOLD)[0]]
    #
    # unbalanced_features_SMD_df = pd.DataFrame(index=range(len(Unbalanced_features_SMD_names)))
    # unbalanced_features_SMD_df = unbalanced_features_SMD_df.assign(Unbalanced_features = list(Unbalanced_features_SMD_names))
    # unbalanced_features_SMD_df = unbalanced_features_SMD_df.assign(SMD=list(Unbalanced_features_SMD_value))
    # unbalanced_features_SMD_df.to_csv(r'K:\WorkArea-zhx2005\PycharmProjects\first_AD\output\35827_result\\unbalanced_features_SMD_df.csv',index=False)
    #
    #
    # Unbalanced_features_IPTW_SMD_value = SMD_ALL[4][SMD_ALL[4] > SMD_THRESHOLD]
    # Unbalanced_features_IPTW_SMD_names = feature_name[np.where(SMD_ALL[4] > SMD_THRESHOLD)[0]]
    #
    # unbalanced_features_IPTW_SMD_df = pd.DataFrame(index=range(len(Unbalanced_features_IPTW_SMD_names)))
    # unbalanced_features_IPTW_SMD_df = unbalanced_features_IPTW_SMD_df.assign(Unbalanced_features = list(Unbalanced_features_IPTW_SMD_names))
    # unbalanced_features_IPTW_SMD_df = unbalanced_features_IPTW_SMD_df.assign(SMD=list(Unbalanced_features_IPTW_SMD_value))
    # unbalanced_features_IPTW_SMD_df.to_csv(r'K:\WorkArea-zhx2005\PycharmProjects\first_AD\output\35827_result\\unbalanced_features_IPTW_SMD_df.csv',index=False)
