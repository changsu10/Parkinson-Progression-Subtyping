...
def get_user_dx_and_outcome(indir, patient_set, icd9_to_ccs, icd10_to_ccs, icd_to_ccw, coding, criterion, ND_code_pkl):
    ...
    with open(indir, 'r') as f:
        ...
        for row in tqdm(csv.reader(f)):  # , total=34554738):
            ...
            for key, fc in criterion.items():
                if fc(dx,ND_code_pkl): # fc(dx) --> fc(dx,ND_code_pkl)
...
def get_user_cohort_dx(indir, prescription_taken_by_patient, icd9_to_css, icd10_to_css, icd_to_ccw, coding,
                       min_patient, patient_set, criterion, ND_code_pkl):
    print('get_user_cohort_dx...')
    print('Diagnosis coding type: ', coding)
    user_dx, cohort_outcome = get_user_dx_and_outcome(indir, patient_set, icd9_to_css, icd10_to_css, icd_to_ccw, coding, criterion, ND_code_pkl)
