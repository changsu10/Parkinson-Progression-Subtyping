...
def parse_args():
    ...
    parser.add_argument('--input_file',
                        ...
                        # default=r'K:\WorkArea-zhx2005\AD_output\testing\prescribing_samples.csv',
                        default=r'K:\dcore-prj0166-SOURCE\wang_adrd\med_admin.csv,K:\dcore-prj0166-SOURCE\wang_adrd\prescribing.csv' #K:\dcore-prj0166-SOURCE\wang_adrd\med_admin.csv,  ,K:\dcore-prj0166-SOURCE\wang_adrd\prescribing.csv
                        )
...
def pre_drug_table(infile):
    ...
    insight:
    prescribing
        ssid, rx_start_date, rxnorm_cui, rx_days_supply, rx_name # Note that, we need change rx_source to raw_rx_med_name

        missing:
        total num records: 41,544,454
        missing for each column
        ssid                             0
        encounterid               41544454
        rx_providerid             41544454
        rx_order_date                  307
        rx_order_time             34567490
        rx_start_date                    2
        rx_end_date               12485185
        rx_dose_ordered           37767072
        rx_dose_ordered_unit      37806165
        rx_quantity               22235886
        rx_dose_form              41544454
        rx_refills                21511204
        rx_days_supply            41544454
        rx_frequency              41544454
        rx_prn_flag               41544454
        rx_route                  41544454
        rx_basis                       614
        rxnorm_cui                  348642
        rx_source                        0
        rx_dispense_as_written    41544454

    med_admin
        ssid,encounterid,prescribingid,medadmin_providerid,medadmin_start_date,medadmin_start_time,medadmin_stop_date,medadmin_stop_time,medadmin_type,medadmin_code,medadmin_dose_admin,medadmin_dose_admin_unit,medadmin_route,medadmin_source

        missing
            ssid                               0
            encounterid                        0
            prescribingid               31027483
            medadmin_providerid         31027483
            medadmin_start_date                0
            medadmin_start_time                0
            medadmin_stop_date          10734557
            medadmin_stop_time           5861742
            medadmin_type                      0
            medadmin_code                      0
            medadmin_dose_admin         31027483
            medadmin_dose_admin_unit    31027483
            medadmin_route               4776964
            medadmin_source                    0
    ...
    with open(infile, 'r', encoding='utf-8') as f:
        ...
        elif 'rxnorm_cui' in col_name:
            drug_2_ing = lambda x: rxnorm_to_ing.get(x)
            print('Using rxnorm_cui to rxnorm_cui ingredient')
            flag_med = 'prescribing'
        elif 'medadmin_code' in col_name:
            drug_2_ing = lambda x: rxnorm_to_ing.get(x)
            print('On medadmin.csv, Using rxnorm_cui to rxnorm_cui ingredient')
            flag_med = 'medadmin'
        elif 'gpi' in col_name:
        ...
        for row in csv.reader(f):
            if row_num_record%100000 ==0:
                print('row_num=',row_num_record)
            row_num_record = row_num_record +1
            n_records += 1
            if flag_med == 'prescribing':
                patid, rx_start_date, rx_end_date, drug = row[0], row[5], row[6], row[17]
            else:
                patid, rx_start_date, rx_end_date, drug = row[0], row[4], row[6], row[9]
            name = '' # drug name
            if rx_start_date:
                date = rx_start_date
            if rx_start_date and rx_end_date:
                day = (str_to_datetime(rx_end_date) - str_to_datetime(rx_start_date)).days
            else:
                day = -1
                n_no_day_supply += 1
            ing = drug_2_ing(drug)
...
if __name__ == '__main__':
    ...
    mci_drug_taken_by_patient = generate_and_dump_drug_patient_records(args.input_file,
                                                                       r'output/PD_drug_taken_by_patient.pkl')
    if 'marketscan' in args.input_file.lower():
        vocab = 'gpi'
    else:
        vocab = 'rxnorm'
    pd_drug = count_drug_frequency(r'output/PD_drug_taken_by_patient.pkl', 'output/drug_prevalence.csv', vocab=vocab)
    print('Done! Time used:', time.strftime("%H:%M:%S", time.gmtime(time.time() - start_time)))
