...
def exclude(prescription_taken_by_patient, patient_dates, eligibility_criteria):
    ...
    for drug, taken_by_patient in prescription_taken_by_patient.items():
        for patient, take_times in taken_by_patient.items():
            ...
            # AD patients cohort----patient_dates: {0: 'birth_date',  1: '1st_diagnosis_date', 2: '1st_mci_date', 3: '1st_AD_date',
            # 4: '1st_dementia_date',  5: '1st_ADRD_date',  6: 'last_diagnosis_date'}
            # PD patients cohort----patient_dates: {0: 'birth_date',  1: '1st_diagnosis_date', 2: '1st_mci_date', 3: '1st_PD_date',
            # 4: '1st_ND_date',  5: '1st_PDRD_date',  6: 'last_diagnosis_date', 7: '1st_CI_date'}

            if patient not in patient_dates:
                continue

            birth_date = patient_dates[patient][0]
            if pd.isna(patient_dates[patient][4]): # [5] 1st_ADRD_date --> [4] 1st_ND_date
                first_adrd_date = datetime.max
            else:
                first_adrd_date = patient_dates[patient][4] # [5] 1st_ADRD_date --> [4] 1st_ND_date
            # DX = patient_1stDX_date.get(patient, datetime.max)
            if pd.isna(patient_dates[patient][3]):  # [2] 1st_mci_date --> [3] 1st_PD_date
                initial_date = datetime.max
            else:
                initial_date = patient_dates[patient][3]  # [2] 1st_mci_date --> [3] 1st_PD_date
            ...
            if criteria_0_is_valid(initial_date, birth_date, eligibility_criteria) and \
                    ...
                    criteria_3_is_valid(index_date, start_date, eligibility_criteria, initial_date) and \
...
def criteria_1_is_valid(index_date, initial_date, eligibility_criteria):
    ...
    # The first prescription is after the PD initiation date, and
...
def criteria_2_is_valid(dates, dates_days, eligibility_criteria):
    ...
    if len(dates) < min_prescription:  # zhenxing revised for chang su project
        return False

    return True


def criteria_3_is_valid(index_date, start_date, eligibility_criteria, initial_date):
    ...
    baseline_PD = 0*30 # 6 month  ---for PD criteria--6 months before PD initiation date to index date  ; 0 month
    return (initial_date - start_date).days >= baseline_PD and (index_date - initial_date).days > 0


def criteria_4_is_valid(index_date, first_adrd_date, eligibility_criteria):
    # Exclude patient who had diagnoses of PD/dementia/ fall/ mental /PIGD before the index date and 3 months after the index date
