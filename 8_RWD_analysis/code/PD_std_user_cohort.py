...
def pre_user_cohort_triplet(prescription_taken_by_patient, user_cohort_rx, user_cohort_dx,
    ...
    for drug, taken_by_patient in tqdm(user_cohort_rx.items()):
        ...
        drug_patient = pd.DataFrame(0,
                                    columns=['bdate', 'sex', 'race', 'PD_date', 'age_of_PD', 'ND_date', 'ND', #  'MCI_date', 'age_of_MCI', 'AD_date', 'AD', ---> 'PD_date', 'age_of_PD', 'ND_date', 'ND',
        ...
        drug_patient['PD_date'] = np.nan #
        drug_patient['age_of_PD'] = np.nan
        drug_patient['ND_date'] = np.nan
        n_no_drug = 0
        n_no_diagnosis = 0
        for patient, taken_times in taken_by_patient.items():
            ...
            index_date_last = drug_dates[-1]# last treatment, end of treatment

            if patient not in patient_dates:
                continue

            followup = (index_date_last - index_date).days +3*30 # index_data to 3 months after end of treatment

            lastdx_date = patient_dates[patient][6]  #
            mci_date = patient_dates[patient][3]  # n note that, 'MCI_date' ---> 'PD_date',  patient_dates[patient][2] -->  patient_dates[patient][3]
            bdate = patient_dates[patient][0]  #
            ...
            if (not pd.isna(mci_date)) and (not pd.isna(bdate)):
                drug_patient.at[patient, 'PD_date'] = mci_date #  drug_patient.at[patient, 'MCI_date'] = mci_date ---->  drug_patient.at[patient, 'PD_date'] = mci_date
                drug_patient.at[patient, 'age_of_PD'] = mci_date.year - bdate.year  # (mci_date - bdate).days / 365.   drug_patient.at[patient, 'age_of_MCI'] = mci_date.year - bdate.year  ---drug_patient.at[patient, 'age_of_PD'] = mci_date.year - bdate.year

            drug_patient.at[patient, 'ND_date'] = outcome_t2e #  'AD_date'  -->  'ND_date'
            drug_patient.at[patient, 'ND'] = outcome #  'AD'  -->  'ND'
