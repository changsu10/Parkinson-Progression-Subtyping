...
# Subset metadata to exclude subtypes II/III.
...
dds$Subtype <- factor(dds$Subtype, levels=c('Control', 'I'))

# Differential expression analysis.
dds <- DESeq(dds)
de_results <- results(dds, 
                      #alpha = 0.05, # Adjusted p-value threshold for significance.
                      contrast = c("Subtype","I","Control")) # Which level as denominator? Case then control.
de_results
...
write.csv(de_results, 'dge_i_vs_hc.csv')
```
